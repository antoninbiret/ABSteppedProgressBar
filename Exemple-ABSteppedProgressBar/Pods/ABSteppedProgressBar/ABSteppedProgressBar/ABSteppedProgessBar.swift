...
@objc protocol ABSteppedProgressBarDelegate {
...
@IBDesignable class ABSteppedProgressBar: UIView {
    ...
    @IBInspectable var progressRadius: CGFloat = 25.0 {
        didSet {
    ...
    @IBInspectable var progressLineHeight: CGFloat = 10.0 {
        didSet {
            self.setNeedsDisplay()
        }
    }
    
    @IBInspectable var stepAnimationDuration: CFTimeInterval = 0.4
    
    @IBInspectable var displayNumbers: Bool = true {
        didSet {
            self.setNeedsDisplay()
        }
    }
    
    var numbersFont: UIFont? {
        didSet {
            self.setNeedsDisplay()
        }
    }
    ...
    @IBInspectable var backgroundShapeColor: UIColor = UIColor(red: 166.0/255.0, green: 160.0/255.0, blue: 151.0/255.0, alpha: 0.8) {
        didSet {
            self.setNeedsDisplay()
        }
    }
    
    @IBInspectable var selectedBackgoundColor: UIColor = UIColor(red: 150.0/255.0, green: 24.0/255.0, blue: 33.0/255.0, alpha: 1.0) {
        didSet {
            self.setNeedsDisplay()
        }
    }
    ...
    override func drawRect(rect: CGRect) {
        ...
        if(!animationRendering) {
            ...
            if let bgPath = shapePath(centerPoints, aRadius: radius, aLineHeight: lineHeight) {
                backgroundLayer.path = bgPath.CGPath
                backgroundLayer.fillColor = backgroundShapeColor.CGColor
            }
        
            if let progressPath = shapePath(centerPoints, aRadius: progressRadius, aLineHeight: progressLineHeight) {
            ...
            if(displayNumbers) {
                for i in 0...(numberOfPoints - 1) {
                    ...
                    var textLayerFont = UIFont.boldSystemFontOfSize(progressRadius)
    ...
    func progressMaskPath(currentProgressCenterPoint: CGPoint) -> UIBezierPath {
        var maskPath = UIBezierPath(rect: CGRectMake(0.0, 0.0, currentProgressCenterPoint.x + progressRadius, self.bounds.height))
        return maskPath
    }
    
    func maskPath(currentProgressCenterPoint: CGPoint) -> UIBezierPath {
        
        let angle = progressLineHeight / 2.0 / progressRadius;
        let xOffset = cos(angle) * progressRadius
        ...
        maskPath.addLineToPoint(CGPointMake(currentProgressCenterPoint.x + xOffset, currentProgressCenterPoint.y - progressLineHeight))
        
        maskPath.addArcWithCenter(currentProgressCenterPoint, radius: progressRadius, startAngle: -angle, endAngle: angle, clockwise: true)
